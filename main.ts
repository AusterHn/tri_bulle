...
let machine_ligneCourante = 0 //commence à 0 parceque la simulation n'a pas encore débuté
let machine_t : number[] = []
let machine_trié : boolean = false
let machine_i : number = 0
let machine_n = 0

//initialisation d'une machine
function initialiserVM(tailleTableau : number) {
...
function echanger(tab : number[], i : number, j : number) {
    let tmp = tab[i]
    tab[i] = tab[j]
    tab[j] = tmp
}
...
function displayAlgorithm() {
    ...
    for (let i = 0; i < algo.length; i++) {
        ...
        if (machine_ligneCourante == i + 1) {
            ...
            let textWidth = ctx.measureText(algo[i]).width
...
function displayVar(x : number, y : number, val : string, title : string) {
    const varDX = 50, varDY = 30
    const valVarDX = 25, valVarDY = 20
    const titleVarDX = 25, titleVarDY = 47
...
function displayVM() {
    const width = 170/machine_t.length //calcul de la largeur de chacune des bandes du tableau
    const max_heigth = 330
    const coeff = 3 //représente le coefficient
    ...
    for (let i = 0; i < machine_t.length; i++) {
        ...
        const width = 680 / machine_t.length // Espace total divisé par le nombre d'éléments
    
        // Calcul de la hauteur proportionnelle
        const height = Math.min(machine_t[i]*coeff, max_heigth)
    
        // Calcul des coordonnées x et y
        const x = 40 + i * width // Position x en fonction de l'index i
        const y = 330 - height // Pour que le rectangle "monte" depuis la base 380
        ...
        ctx.font = "16px Arial"
        ctx.textAlign = "center"
        ctx.fillText(machine_t[i].toString(), x + width / 2, y + height / 2)
        ctx.fillStyle = "black"
        ctx.fillText("t["+(i+1)+"]", x + width / 2, 360)
    }
...
const champTailleTab = <HTMLInputElement> document.getElementById("taille_tableau")
const champVitesseSim = <HTMLInputElement> document.getElementById("vitesse_simulation")
const boutonInit = <HTMLButtonElement> document.getElementById("initialiser")
const boutonStep = <HTMLButtonElement>document.getElementById("un_pas")
const boutonContinue = <HTMLButtonElement> document.getElementById("continuer")
const boutonBreak = <HTMLButtonElement> document.getElementById("stop")
...
function reinit() {
    ...
    let nb = parseInt(champTailleTab.value, 10)
...
function simContinue() {
    ...
    let vitesse : number = parseFloat(champVitesseSim.value)
    if (!simArrete) {
        makeaStep()
        refreshCanevas()
